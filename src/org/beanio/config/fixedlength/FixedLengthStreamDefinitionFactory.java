...
public class FixedLengthStreamDefinitionFactory extends StreamDefinitionFactory {
	...
	protected void compileFieldDefinitions(RecordConfig recordConfig,
		...
		for (int i=0; i<fieldCount; i++) {
			...
			width[i] = fieldArray[i].getWidth();
			
			if (width[i] == 0) {
				throw new BeanIOConfigurationException(
					"Invalid width on field '" + fieldArray[i].getName() + "'");
			}
		}
		if (position[0] < 0) {
			position[0] = 0;
		...
		for (int i=0; i<fieldCount; i++){
			...
			if (width[i] < 0) {
				...
				if (width[i] < 0) {
					throw new BeanIOConfigurationException(
						"width not set on field '" + fieldArray[i].getName() + "'");
				}
			}
		}
		...
		for (FieldDefinition fieldDefinition : recordDefinition.getFieldList()) {
			...
			flFieldDefinitin.setWidth(width[index]);
			...
			recordWidth += flFieldDefinitin.getWidth();
			++index;
		}
		
		// TODO 
		flRecordDefinition.setMinLength(recordWidth);
		flRecordDefinition.setMaxLength(recordWidth);
	}
