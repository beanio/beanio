...
public class StreamCompiler {
     ...
     * @throws IOException                  if an I/O error occurs reading the
     *                                      mapping file
     ...
     * @return the new {@link TypeHandlerFactory}, or <code>parent</code> if the
     *         configuration list was empty
     * @since 2.0
     */
    private TypeHandlerFactory createTypeHandlerFactory(TypeHandlerFactory parent, List<TypeHandlerConfig> configList) {
        ...
        for (TypeHandlerConfig hc : configList) {
            ...
            if (hc.getType() != null) {
                try {
                    // type handlers configured for java types may be registered for a specific
                    // stream format
